...
rmse = tf.sqrt(cost_pred)

weights = [conv1_w, conv2_w, conv3_w, conv4_w, fc_w, w_out]
...
with tf.Session() as sess:
    ...
    for iteration in range(n_iterations // display_step):
        ...
        c = sess.run(cost, feed_dict={X: x_batch_c, Y: y_batch_c})
        pred_train = sess.run(pred, feed_dict={X: x_batch_c, Y: y_batch_c})
        pred_val = sess.run(pred, feed_dict={X: x_batch_v, Y: y_batch_v})
